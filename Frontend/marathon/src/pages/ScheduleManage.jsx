  ...
  const { isLoading, data, isError, error } = useQuery(
    ...
    () => $.get(`/doctor-treatment/table`),
    {
      onSuccess: (data) => {
        ...
        setTeacherSchedule(data.data.list);
        console.log(teacherSchedule);
        const thisDateNum = data.data.firstDateInfo;

        const totalThisDate = (num) => {
          let today = new Date(
            Number(thisDateNum) + (num + cnt * 7) * 86400000
          );
        ...
        const onChange = (e) => {
          ...
          let findItem = teacherSchedule.find(
            (t) => t.localDate === e.target.name
          );
          let newTimeTable = [];
          for (let i = 0; i < 8; i++) {
          ...
          let findorigin = newSchedule.find(
            (t) => t.localDate === e.target.name
          );
